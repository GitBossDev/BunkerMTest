...
# ── Default ACL Access endpoints ────────────────────────────────────────────

DEFAULT_ACL_TYPES = ["publishClientSend", "publishClientReceive", "subscribe", "unsubscribe"]
...
@app.get("/api/v1/default-acl")
async def get_default_acl(request: Request, api_key: str = Security(get_api_key)):
    """Get the default ACL access settings"""
    await log_request(request)
    success, result = execute_mosquitto_command(["getDefaultACLAccess"])
    if not success:
        raise HTTPException(status_code=500, detail=result)

    # Parse output lines like: "publishClientSend    : allow"
    ...
    for line in result.splitlines():
        ...
        if " : " in line:
            ...
            key = key.strip()
            value = value.strip()
            if key in DEFAULT_ACL_TYPES:
                config[key] = value == "allow"

    if not config:
        raise HTTPException(status_code=500, detail="Failed to parse defaultACLAccess")

    return config


@app.put("/api/v1/default-acl")
async def set_default_acl(
    acl_config: DefaultACLConfig,
    request: Request,
    api_key: str = Security(get_api_key),
):
    """Update the default ACL access settings"""
    await log_request(request)
    updates = {
    ...
    for acl_type, allow in updates.items():
        ...
        success, result = execute_mosquitto_command(["setDefaultACLAccess", acl_type, value])
        if not success:
            errors.append(f"{acl_type}: {result}")
...
def _mqtt_topic_matches(acl_pattern: str, test_topic: str) -> bool:
    """Check if test_topic matches the ACL topic pattern (MQTT wildcard rules)."""
    pp = acl_pattern.split('/')
    tp = test_topic.split('/')

    def _match(pi: int, ti: int) -> bool:
        if pi == len(pp) and ti == len(tp):
            return True
        if pi < len(pp) and pp[pi] == '#':
            return True
        if pi >= len(pp) or ti >= len(tp):
            return False
        if pp[pi] == '+' or pp[pi] == tp[ti]:
...
class ACLTestRequest(BaseModel):
    topic: str = Field(..., description="Topic to test subscription on")
    aclType: str = Field(..., description="ACL type to test (e.g. subscribePattern)")


@app.post("/api/v1/roles/{role_name}/acls/test")
async def test_role_acl(
    ...
    """
    Test whether a role grants access for a given topic and ACL type.
    Reads dynamic-security.json directly and applies Mosquitto's ACL evaluation
    logic (priority order → first match wins → fallback to defaultACLAccess).
    """
    await log_request(request)
    dynsec_path = os.getenv("DYNSEC_PATH", "/var/lib/mosquitto/dynamic-security.json")
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Cannot read dynsec config: {e}")

    # Find role
    roles_list = dynsec.get("roles", [])
    role_data = next((r for r in roles_list if r.get("rolename") == role_name), None)
    ...
    sorted_acls = sorted(acls, key=lambda a: a.get("priority", 0), reverse=True)

    # Find first matching rule for the requested aclType + topic
    matched_rule = None
    for acl in sorted_acls:
        if acl.get("acltype") != body.aclType:
            continue
        if _mqtt_topic_matches(acl.get("topic", ""), body.topic):
            matched_rule = acl
            break

    if matched_rule is not None:
        allowed = matched_rule.get("allow", False)
        return {
            "allowed": allowed,
            "reason": "role_acl",
            "matchedRule": {
                "topic": matched_rule.get("topic"),
                "aclType": matched_rule.get("acltype"),
                "allow": allowed,
                "priority": matched_rule.get("priority", 0),
            },
        }

    # No role rule matched — check defaultACLAccess fallback
    default_acl = dynsec.get("defaultACLAccess", {})
    # Map aclType names to defaultACLAccess keys
    type_to_default = {
    ...
    default_key = type_to_default.get(body.aclType)
    fallback_allowed = default_acl.get(default_key, False) if default_key else False
