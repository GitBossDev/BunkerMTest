...
def _include_object(object_, name, type_, reflected, compare_to):
    if type_ == "table":
        return name in HISTORY_REPORTING_TABLE_NAMES
    table_name = getattr(getattr(object_, "table", None), "name", None)
    if table_name is not None:
        return table_name in HISTORY_REPORTING_TABLE_NAMES
    return True
...
def _version_table() -> str:
    return config.get_main_option("version_table") or HISTORY_REPORTING_VERSION_TABLE
