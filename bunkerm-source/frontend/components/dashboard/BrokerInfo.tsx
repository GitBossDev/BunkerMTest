...
/** Normalise uptime e.g. "1234 seconds" → "1234 s" or keep as-is */
function parseUptime(raw?: string): string {
  if (!raw) return '—'
  // Mosquitto reports uptime as "N seconds"
...
export function BrokerInfo({ stats }: BrokerInfoProps) {
  ...
  const clients   = stats?.total_connected_clients ?? 0
  const total     = stats?.clients_total ?? clients
  const maximum   = stats?.clients_maximum ?? clients
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-sm font-medium">Broker Info</CardTitle>
      ...
      <CardContent>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <Stat label="Connected"  value={String(clients)}  accent="text-blue-500" />
          <Stat label="Total"      value={String(total)}    />
          <Stat label="Max ever"   value={String(maximum)}  accent="text-orange-500" />
          <Stat label="Subs / Ret" value={`${subs} / ${retained}`} />
        </div>
      </CardContent>
    </Card>
  )
